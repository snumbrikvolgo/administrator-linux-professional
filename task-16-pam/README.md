...
```powershell
cd C:\path\to\administrator-linux-professional\task-16-pam
vagrant up
```
...
После запуска VM перейти в эту же папку из WSL:

```bash
cd /mnt/c/path/to/administrator-linux-professional/task-16-pam
...
```bash
$ id otus
uid=1002(otus) gid=1003(otus) groups=1003(otus)

$ id otusadm
uid=1003(otusadm) gid=1004(otusadm) groups=1004(otusadm),1001(admin)

$ id dockeradm
uid=1004(dockeradm) gid=1005(dockeradm) groups=1005(dockeradm),110(docker)
```
...
Запуск только тестов после настройки стенда:

```bash
ansible-playbook -i ansible/inventory.ini ansible/playbook.yml --tags test
```
...
Команда выполняется без запроса пароля.
